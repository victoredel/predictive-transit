...
# Importamos las clases generadas para GTFS-Realtime
...
class MbtaAdapter(BaseTransitAdapter):
    ...
    async def fetch_and_normalize(
        ...
        """
        1. Hace la petición GET al endpoint público MBTA.
        2. Parsea la respuesta binaria Protobuf.
        3. Convierte las entidades a la clase NormalizedPosition.
        """
        try:
        ...
        # Parseo del contenido binario (Protobuf)
        ...
        # Normalización a nuestro formato estándar
        ...
        for entity in feed.entity:
            ...
            # Map VehicleStopStatus (enum in protobuf) to string if possible, or stringify enum number
            status_str = "IN_TRANSIT_TO" # default sensible
            if vp.HasField("current_status"):
                if vp.current_status == gtfs_realtime_pb2.VehiclePosition.IN_TRANSIT_TO:
                    status_str = "IN_TRANSIT_TO"
                elif vp.current_status == gtfs_realtime_pb2.VehiclePosition.STOPPED_AT:
...
async def publish_to_stream(
    ...
    """
    Toma los diccionarios limpios (NormalizedPosition) e inyecta al stream redis.
    """
    published = 0
    for record in positions:
        try:
            await redis_client.xadd(
                STREAM_NAME,
                record,  # Type checker puede alertar si no es un dict-like de strings
...
async def polling_loop() -> None:
    ...
    Mantiene la conexión Redis viva, inicializa el adaptador elegido y
    realiza el pipeline de lectura/normalización/escritura en loop eterno.
    """
    logger.info("🚌 Real-Time Ingestion arrancando (Adapter Mode) …")
    logger.info("   Redis URL    : %s", REDIS_URL)
    logger.info("   Poll interval: %ds", POLL_INTERVAL)
    
    # 1. Instanciamos el adaptador que queremos usar activamente.
    #    (Fácilmente reemplazable por cualquier subclase de BaseTransitAdapter)
    adapter: BaseTransitAdapter = MbtaAdapter()
    
    logger.info("   Adaptador act: %s", adapter.__class__.__name__)

    # Reusamos ClientSession (importante para mantener pools de conexiones ligeros)
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            # ── B. Ejecución de la Estrategia (Fetch & Normalize)
            positions = await adapter.fetch_and_normalize(session)
            
            logger.info(
                "[%s] Vehículos parseados: %d", 
                adapter.__class__.__name__, 
                len(positions)
            )

            # ── C. Publicación de Resultados (Si hay info y conexión)
            ...
            # ── D. Esperamos al siguiente turno
            elapsed = asyncio.get_event_loop().time() - cycle_start
            sleep_time = max(0.0, POLL_INTERVAL - elapsed)
            logger.debug("Completado en %.2fs. Durmiendo %.2fs.", elapsed, sleep_time)
